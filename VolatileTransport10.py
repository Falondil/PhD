...
number_of_compression_timesteps = 10
...
pebblerate = 1e-0*1e6 # Pg yr-1, PLACEHOLDER VALUE, x order of magnitude less than what is required to form a 1 Earth mass planet over 5 Myr.
...
def temperaturefunc(rvar, drvar, pressure, Menc): # calculate the temperature as a function of pressure and enclosed mass 
    ...
    return np.append((Tsurf+deltaT)*np.flip(np.exp(exponent)), Tsurf+deltaT) # return the temperature at each layerboundary (including the surface)
...
def massanddensityinterpolation(r, dr, rnew, newdensity): # interpolate the density from the new layer centers to the linearly spaced layer centers
    ...
    drnew = delta(rnew) # layer thicknesses
...
initialpressure0 = psurf + GPaconversion*2*pi/3*silicatedensity**2*G*(rsurf**2-rk**2) # SOMETHING IS WRONG HERE. initial pressure in the magma ocean layerboundaries resulting from constant density profile, GPa
initialpressure = pressurefunc2(rk, delta(rk), initialdensity, Menc)
...
for j in range(number_of_compression_timesteps):
    density = densityfunc(pressure, temperature)
    rkvar = newlayerboundaries(Mtot, density)
    if fixedplanetradius: # Not functioning yet
        pointstoadd = int((rsurf-max(rkvar))/dr)+1 # how many layer boundaries have to be added to reach the planet's surface
        rkvar = np.concatenate((rkvar, np.array([max(rkvar)+k*dr for k in range(1, pointstoadd)])))
        rkvar = np.append(rkvar, rsurf) # add the surface point
        Mtot = np.concatenate((Mtot, silicatedensity*4*pi*delta(rkvar[-pointstoadd-1:]**3))) # Really wrong for some reason. I don't know why. 
        Menc = menc(Mtot)
        density = np.concatenate((density, np.repeat(silicatedensity, pointstoadd)))
    drkvar = delta(rkvar)
    pressure = pressurefunc2(rkvar, drkvar, density, Menc)
    temperature = temperaturefunc(rkvar, drkvar, pressure, Menc)
    ...
    if j%plotstepspacing==0 and compressionplotting:      
        ...
        ax1.plot(rkvar, densityconversion*density, color=colors[0])
        ax1.tick_params(axis='y', labelcolor=colors[0])
        
        ax3 = ax1.twinx() # shared x-axis
        ax3.set_ylabel('Enclosed mass [Pg]', color=colors[1])  
        ax3.plot(rkvar, Menc, color=colors[1])
        ax3.tick_params(axis='y', labelcolor=colors[1])
        
        color = 'tab:blue'
        ax2.set_xlabel('radius [km]')
        ax2.set_ylabel('Pressure [GPa]', color=colors[2])
        ax2.plot(rkvar, pressure, color=colors[2])
        ax2.tick_params(axis='y', labelcolor=colors[2])
        
        ax4 = ax2.twinx() # shared x-axis
        color = 'tab:red'
        ax4.set_ylabel('Temperature [K]', color=colors[3])  
        ax4.plot(rkvar, temperature, color=colors[3])
...
# convection
rk = np.linspace(rc, rsurf, number_of_boundaries)
