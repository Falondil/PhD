...
nosedifrac = 0.02 # for how big fraction of the total time should sedimentation be halted. CHOICE 0 to 1. 
...
def PCfunc(p, T, deltaIW=0, Xoxygen=0, Xsulfur=0, NBOperT=NBOperT, microprobe=False):
...
def PNfunc(p, T, XFeO,  Xsulfur=0, Xsilicon=0, NBOperT=NBOperT):
...
for timestep in range(1, number_of_timesteps+1):
    ...
    if timestep % savingspacing == 0 or timestep == number_of_timesteps-1:
        print('Time elapsed: '+timeelapsedstr+' days. Simulation speed = '+str(currentsimulationspeed)+' yr/s. '+'Time step speed = '+str(timestep/currentlooptimeelapsed)+' s-1')
