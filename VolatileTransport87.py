...
XFeO = 0.1 # . NEEDED FOR PNfunc and CO2 solubility. GET CORRECT CALCULATION OF THIS. 
...
def PNfuncGrewal(p, T, XFeO=XFeO, Xsulfur=XS, Xsilicon=0, NBOperT=NBOperT):
    if cappedtemperature:
...
# WORK MORE
def startingatmosphere2(Xiktop, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True):
    ...
    pioxisol = newton(pioxiroot, psurf*np.ones(3)/3) # arbitrary starting guess: all pi are equal
    ...
    return Xiatmfull*Matm, np.sum(pifullsol)

def iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True, number_of_iterations=100): # computes starting atmosphere until the total mass of each volatile matches the prescribed initial inventory (Mitot0)
    iterXi = np.copy(Xiguess)
    for j in range(number_of_iterations):
        Miatmfull, psurfnew = startingatmosphere2(iterXi, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen)
        ...
        iterXi *= factor
    return Miatmfull[:3], Miatmfull[-2:], iterXi, psurfnew
...
def Miatmeqfunc4(Miatmguess, Miatmextraguess, amui, amuiextra, psurf, Msiltop, Mitop0, differentnitrogen=True, method='bisect'):
    ...
    Mitopeq = Miatmequiv0+Mitop0-Miatmeqequiv # INCORRECT FOR NOW FOR SOME REASON. LINES AFTER "Miatmeq = ... " HAVE AN ERROR
    return Miatmeq, Mitopeq 
...
# print('Xi0', Xi0)
Miatmguess, Miatmextraguess, Xi0, psurf = iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf0, protomass, rsurf, psurf, Tsurf)
# print('Xi0', Xi0)
Mvol0 = np.array([M*Xi0 for M in Msil]).transpose()
Miatm0, Miatmextra0 = Miatmguess, Miatmextraguess

piatm0 = psurf*molefraction(np.concatenate((Miatm0, Miatmextra0)), amuifull)
