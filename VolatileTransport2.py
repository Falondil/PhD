...
Mvol = np.array([[f0*M for M in M0] for f0 in fi0]) # total mass of volatiles in each layer, Mik, [Pg]. PLACEHOLDER
...
def massfromconvection(masses, sigmak, metsilfrac=0, Pik=0): # function for computing new masses (of any element) in each magma ocean sublayer resulting from convection after a timestep dt 
    ...
    if truncnormdist:
        Mkprimek *= masses*(metsilfrac*Pik+1)**(-1)/np.sum(Mkprimek, axis=0) # sum over all k' and upscale so that mass is conserved. FLAG HERE
...
for j in range(number_of_timesteps):
    ...
    fig.suptitle('Mass in magma ocean layers after '+str(j+1)+' convection time steps', y=0.85)
    ...
    # Plot masses
    ax1.set(xlabel='Distance from planet center [km]', ylabel=('Silicate mass [Pg]'), aspect=3/4*(rsurf-rc)/max(Msil))
    ax1.plot(layercenters, M0, '-.', color='k', label='initial silicate mass')
    ...
    # Plot densities
    # fig.suptitle('Density in magma ocean layers after '+str(j+1)+' convection time steps', y=0.85)
    # ax1.set(xlabel='Distance from planet center [km]', ylabel=('Silicate density [Pg km-3]'), aspect=3/4*(rsurf-rc)/max(Msil/layervolumes))
    # ax1.plot(layercenters, M0/layervolumes, '-.', color='k', label='initial silicate density')
