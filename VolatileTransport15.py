...
Tsurf = 1500 # surface temperature, K, PLACEHOLDER
...
def Cpfunc(temperature, material='e'): # Shomate equation for specific heat capacity of material as function of temperature. NIST Webbook, Condensed phase thermochemistry data
    ...
    if material=='e': # enstatite, MgSiO3
        ...
        if temperature>1850: # 1850 K < T
            a, b, c, d, e = 146.4400, -1.499926e-7, 6.220145e-8, -8.733222e-9, -3.144171e-8
        elif temperature>903: # 903 K < T < 1850 K
            a, b, c, d, e = 37.72742, 110.1852, -50.79836, 7.968637, 15.16081
        else:
            a, b, c, d, e = 91.93210, 33.63681, -0.677348, 0.289529, -1.776498
    elif material=='f': # forsterite, Mg2Si04
        molecularweight = 140.6931/1000 # kg/mol
        if temperature>2171: # 2171 K < T
            a, b, c, d, e = 205.0164, -2.196956e-7, 6.630888e-8, -6.834356e-9, -1.298099e-7
        else: # T < 2171 K
            a, b, c, d, e = 140.8749, 47.15201, -12.22770, 1.721771, -3.147210
    Cpermole = a+b*Tper1000+c*Tper1000**2+d*Tper1000**3+e/Tper1000**2 # Shomate equation, J/(mol*K)
...
def temperaturefunc2(rvar, drvar, pressure, Menc, Cp=Cpsil): # TENSION BETWEEN FUNC AND FUNC2. Layer step-wise calculating the temperature as a function of pressure and enclosed mass. 
    rflip = np.flip(rvar[1:]) # flip the array of layerboundaries so it is ordered from the top down. r=rc excluded so first element cut before flip.
    drflip = np.flip(drvar) 
    Mencflip = np.flip(Menc[1:]) # also flip the enclosed mass array
    pressureflip = np.flip(centeraveraging(pressure)) # and the pressure array (innermost pressure is not needed)
...
def temperaturefunc3(rvar, drvar, pressure, Menc, Cp=Cpsil): # TENSION BETWEEN FUNC AND FUNC2. Layer step-wise calculating the temperature as a function of pressure and enclosed mass. 
    rflip = np.flip(rvar[1:]) # flip the array of layerboundaries so it is ordered from the top down. r=rc excluded so first element cut before flip.
    drflip = np.flip(drvar) 
    Mencflip = np.flip(Menc[1:]) # also flip the enclosed mass array
    pressureflip = np.flip(centeraveraging(pressure)) # and the pressure array (innermost pressure is not needed)
    ...
    temperatureflip[0] = Tsurf # surface temperature for the first temperature of the adiabat 
    for i in range(1, number_of_structure_boundaries):
        Cpflip = Cpfunc(temperatureflip[i-1]+deltaT, liquid=(temperatureflip[i-1]>1850)) # set the specific heat capacity to dependent on the temperature of the layer above
...
for j in range(number_of_structure_timesteps):
    ...
    temperature = temperaturefunc2(rsvar, drsvar, pressure, Menc)
