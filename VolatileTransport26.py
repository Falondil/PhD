...
def accretionluminosity(accretedmassrate, rvar, drvar, Menc): # calculates the potential energy lost by accretion from infinity to the depth we consider the material deposits its heat
...
def NuKmax(Lmet, Tadcore, Tsurf, rcore, rsurf): # calculates the upper limit for the effective heat conduction due to conduction + convection. (Can't result in smaller temperature drop than for the adiabatic temperature profile)
    return Lmet*(1/rcore-1/rsurf)/(4*pi*(Tadcore-Tsurf))
...
def Nusselt(r, pressure, temperature, density, Menc, deltaTad, convectivedepth): # calculate the Nusselt number
    Ra = Rayleigh(r, pressure, temperature, density, Menc, deltaTad, convectivedepth)
...
def diffusivity(r, pressure, temperature, density, Menc, deltaTad, convectivedepth, K = heatconductivity): # calculate diffusivity
    Nu = Nusselt(r, pressure, temperature, density, Menc, deltaTad, convectivedepth)
    Cp = Cpfunc(temperature)
...
surfacetemp = 2400 # K
NuK = 2e6*heatconductivity # temporary Nusselt number * heat conductivity
excesstemperature = 10 # K, minimum value achievable for deltaTad
...
for j in range(number_of_structure_timesteps):
    ...
    Lmet = accretionluminosity(pebblerate*pebblemetalfrac, rsvar, drsvar, Menc)
    ...
    maxNuK = NuKmax(Lmet, adiabattemperature[0]+excesstemperature, surfacetemp, np.mean(rsvar[:2]), np.mean(rsvar[-2:])) # calculate the maximal Nusselt number that still provides weak enough convection to have a larger temperature gradient between first and last magma ocean layer.
    NuK = np.min([NuK, maxNuK]) # make sure NuK is below its upper limit
    ...
    NuK = np.mean(heatconductivity*Nusselt(rsvar, pressurelong, temperaturelong, densitylong, Menc, deltaTad, rsvar[-1]-rsvar[0]))
