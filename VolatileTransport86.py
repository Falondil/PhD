...
GPatoPa = 1e9 # converts from GPa to Pa
...
def startingatmosphere2(Xiktop, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True):
    ...
    if differentnitrogen:
        alphaioxi[2] = 6.11e-13*1e6 # Sossi2020 and Anatomy2 eq. 12
        betaioxi[2] = 1
...
def iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True, number_of_iterations=100): # computes starting atmosphere until the total mass of each volatile matches the prescribed initial inventory (Mitot0)
    for j in range(number_of_iterations):
        Miatmfull = startingatmosphere2(Xiguess, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen)
        Miatmequiv = Miatmfull[:3]+np.append(Miatmfull[-2:]*amuifull[:2]/amuifull[-2:], 0) # reduced species scale up
        Mimelt = Xiguess*Mmagma
        factor = Mitot0/(Mimelt+Miatmequiv)
        Xiguess *= factor
    return Miatmfull[:3], Miatmfull[-2:], Xiguess
...
def Miatmeqfunc4(Miatmguess, Miatmextraguess, amui, amuiextra, psurf, Msiltop, Mitop0, differentnitrogen=True):
...
Miatmguesstest, Miatmextraguesstest, Xi0test = iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf0, protomass, rsurf, psurf, Tsurf)
...
psurf0 = psurffunc(np.sum(np.concatenate((Miatmguess, Miatmextra0))), rsurf, protomass)
...
for timestep in range(1, number_of_timesteps+1):
    ...
    if timestep % restructuringspacing == 0 or timestep == number_of_timesteps:
        ...
        if growthbool and simulatedyears>settime*nosedifrac: # if we want to grow the planet
            ...
            # CHOICE, add late accretion of volatiles
            for i in irange:
