...
def Kifunc(Matm, rsurf=rsurf, protomass=protomass, meltindex=0): # choice of melt from which experimental K for CO2 is taken
    psurf = psurffunc(Matm, rsurf, protomass)
    pinkbar = psurf*GPatokbar 
    KH2O = 0.1*np.exp(-3+pinkbar/10) # Stolper1982, fig. 4 and eq. 8. Product of K1 and K2. Slopes up one log unit after 10 kbar.
    KCO2withNepheline = np.exp(-12.5-5.5*pinkbar/30) # Spera1980, fig. 1. Minus sign since we want the reverse equilibrium constant
    KCO2withOlivineMelilite = np.exp(-13-5*pinkbar/30) # Spera1980, fig. 3. -||-
    KCO2 = np.array([KCO2withNepheline, KCO2withOlivineMelilite])[meltindex]
    KN2 = KH2O # PLACEHOLDER
    return np.array([KH2O, KCO2, KN2])

# initial mass fraction of volatiles in melt, CHOICE
...
def Miatmeqfunc(Miatm0, AMUi, alphai, Msiltop, Misil0, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        ...
        return 1/Mitot0*(Miatm+(Ki*frac/np.sum(frac))**(1/alphai)*Msiltop)-1 # better to solve for the unitless equation = 0
...
Miatmstart = startingatmosphere(Matm, (Mvol[:,-1]/Msil[-1]), amui, alphai)
