...
settime = 1e6 # years of accretion to simulate
...
formationtime = 5e6 # time needed to form one Earth mass planet
if constantpebblerate:
    pebblerate = 1*earthmass/formationtime # Pg yr-1, PLACEHOLDER VALUE, x order(s) of magnitude less than what is required to form a 1 Earth mass planet over 5 Myr.
else:
    M0 = 1e-4 # 1e-2 I read somewhere(?), 1e-4 from Anders or maybe 1e-6 from Claudia. Should be mass at which mass first starts accreting pebbles (units = Earth masses)
    tau = formationtime/np.log(1/M0)
    pebblerate = massscaling*earthmass/tau
...
def pTrcb(rsurf, pebblerate=pebblerate, protomass=protomass, gamma=atmgamma, a=AU, Lstar=Lsun, beta=2):
...
def pTsurf2(rsurf, Matm=Matmguess, pebblerate=pebblerate, protomass=protomass, gamma=atmgamma, a=AU, Lstar=Lsun):
    prcb, Trcb = pTrcb(rsurf, pebblerate, protomass, gamma, a, Lstar)
    gsurf = G*protomass/rsurf**2
    psurf = gsurf*Matm/(4*pi*rsurf**2)*GPaconversion+prcb # GPa, pressure at surface
    # Tsurf = Trcb*(psurf/prcb)**((gamma-1)/gamma) # Corresponds to earlier pTsurf method. WHY IS THIS WRONG?
    nabla_ad = (gamma-1)/gamma
    mgas = 2.2*mproton
    Tsurf = Trcb+nabla_ad*mgas*2*G*protomass/kB*1/rsurf # T_disk*R_B'/R_rcb term neglected. We assume R_B' is small since our thick atmosphere is not extended.
    return psurf, Tsurf
...
for timestep in range(number_of_timesteps):
    ...
    if (checktime-loopstart)>4.8*86400: # 5 days is maximum cluster time
        break # stop prematurely so that 
