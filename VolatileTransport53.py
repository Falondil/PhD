...
massscaling = 0.3 # how many earth masses to start with. >0.3 seems very safe (deltaTad = negligible), >0.1 might be safe (temperature profile looks plausible).
settime = 2.5e6 # years of accretion to simulate
...
Mcoretimearray = np.zeros((len(volatiles), number_of_timesteps//savingspacing+2)) # create array to fill with total core mass
...
for timestep in range(number_of_timesteps):
    ...
    protomass = mc+Mmagma+np.sum(Miatm)
...
np.savetxt('CMB.out', cmbcondtimearray, delimiter=',') # rcmb, pcmb, Tcmb order
...
np.savetxt('Mcore.out', Mcoretimearray, delimiter=',') # Mcore for each time step
np.savetxt('Mmagma.out', Mmagmatimearray, delimiter=',') # mass of magma ocean for each time step
np.savetxt('protomass.out', protomasstimearray, delimiter=',') # protomass for each time step
