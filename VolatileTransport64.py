...
fullgasvolatiles = np.array(['H2O', 'CO2', 'N2', 'H2', 'CO'])
...
def nabla_ad_atm(temperature, species='CO2', returnCp=False): 
    ...
    if species=='H2O':
        ...
        Cpermole = np.where(temperature<2500, a1+b1*Tper1000+c1*Tper1000**2+d1*Tper1000**3+e1/Tper1000**2,
                             a2+b2*Tper1000+c2*Tper1000**2+d2*Tper1000**3+e2/Tper1000**2)  # Shomate equation, J/(mol*K))
    elif species=='CO':
        ...
        Cpermole = np.where(temperature<1300, a1+b1*Tper1000+c1*Tper1000**2+d1*Tper1000**3+e1/Tper1000**2,
                             a2+b2*Tper1000+c2*Tper1000**2+d2*Tper1000**3+e2/Tper1000**2)  # Shomate equation, J/(mol*K))
    else: 
...
def Miatmeqfunc(Miatmguess, Miatmextra, rsurf, AMUi, AMUiextra, alphai, Msiltop, Misil0, fO2surf, protomass, pebblerate, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at physical+chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        frac = Miatm/AMUi
        fracextra = Miatmextra/AMUiextra
...
def gasgasfunc(Miatm, Miatmextra, amui, amuiextra, fO2, Tsurf, splitbool=True):
    ...
    if splitbool:
        Miatm, Miatmextra = np.split(Miatmfull, [len(Miatm)])
        return Miatm, Miatmextra 
    else:
...
Miatmextraguess = np.zeros(2) # guess no H2 or CO at the start
...
# GASGAS NOT YET WORKING
Miatm0, Miatmextra0 = gasgasfunc(eqstate[0], Miatmextraguess, amui, amuiextra, fO2surf0, Tsurf)
...
for timestep in range(number_of_timesteps):
    ...
    Miatm, Miatmextra = gasgasfunc(Miatm, Miatmextra, amui, amuiextra, fO2surf, Tsurf) 
