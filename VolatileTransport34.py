...
# LEGACY, INCORRECT. IT IS CANCELLED IN THE MASS BALANCE DERIVATION. conversion factor from fraction of NBO that get a volatile attached to them to mass fraction of the volatile atoms inside the melt
# betai = NBOperT*Tperformula/amuMelt*np.array([amuH2O, amuC, amuN]) # PLACEHOLDER, using amuH2O to count H. 
# betai = np.ones(len(volatiles)) # PLACEHOLDER. if the NBO/T stuff is nonsense
...
def startingatmosphere(Matm, Xiktop, AMUi, alphai, betai, Ki):
    XiNBO = Xiktop/betai # fraction of NBO that actually have a volatile i bound to them
    frac = XiNBO**alphai*AMUi/Ki
...
def Miatmeqfunc(Miatm0, AMUi, alphai, betai, Ki, Msiltop, Misil0, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    return Miatmeq, Misileq, meltmassfraci, atmmassfraci, Miatmeq/Miatm0


Miatmstart = startingatmosphere(Matm, (Mvol[:,-1]/Msil[-1]), amui, alphai, betai, Ki)
Miatmeqstart = Miatmeqfunc(Miatmstart, amui, alphai, betai, Ki, Msil[-1], Mvol[:,-1])
