...
for j in range(number_of_compression_timesteps):
    ...
    if j%plotstepspacing==0:      
        ...
        fig.suptitle('Structure of magma ocean after '+str(j)+' timesteps')
        
        color = 'tab:green'
        ax1.set_ylabel('Density [Pg km-3]', color=color)
        ax1.plot(rkvar, density, color=color)
        ax1.tick_params(axis='y', labelcolor=color)
        
        color = 'tab:blue'
        ax2.set_xlabel('radius [km]')
        ax2.set_ylabel('Pressure [GPa]', color=color)
        ax2.plot(rkvar, pressure, color=color)
        ax2.tick_params(axis='y', labelcolor=color)
        
        ax3 = ax2.twinx() # shared x-axis
        color = 'tab:red'
        ax3.set_ylabel('Temperature [K]', color=color)  
        ax3.plot(rkvar, temperature, color=color)
