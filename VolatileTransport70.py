...
def startingatmosphere(Matm, Xiktop, amui, alphai, fO2surf, rsurf, protomass, pebblerate, nitrogenchoice=1):
    ...
    frac = Xiktop**alphaiused*amui/Kiused
...
def Miatmeqfunc(Miatmguess, Miatmextra, rsurf, amui, amuiextra, alphai, Msiltop, Misil0, fO2surf, protomass, pebblerate, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    if method=='fsolve': # appears to be stably solving further-from-equilibrium starting guesses
        Miatmeq, infodict, ier, mesg = fsolve(Miatmrootfunc, Miatmguess, xtol=1e-10, full_output=True, maxfev=400, factor=0.1) # find roots to the function with a starting guess being the unequilibrated atmospheric mass of the volatile species.
...
def gasgasmelt(Miatm, Miatmextra, Miktop, Msiltop, amui, amuiextra, psurf, Tsurf, fO2surf, H2Oindex=0, meltindex=0, method='fsolve'): # handles both dissolution (gas-melt reaction) and gas-gas speciation simultaneously
    ...
    if method=='fsolve': # appears to be stably solving further-from-equilibrium starting guesses
        Miatmeq = fsolve(Miatmrootfunc, Miatm, xtol=1e-8, maxfev=100) # find roots to the function with a starting guess being the unequilibrated atmospheric mass of the volatile species.
    else:
