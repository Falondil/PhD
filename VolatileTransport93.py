...
planetesimalaccretion = True # not working yet. fsolve finds bad solutions
...
MH2Otot0 = Mearthocean*[9,45][0]+770*1e-6*(1-coremassfraction)*earthmass # Huang2026 + Hirschmann2018
MCtot0 = 44/12*((1-coremassfraction)*140*1e-6+coremassfraction*0.3/100)*earthmass # Hirschmann2018 for 140 ppm. Chen2014 for 0.3wt% C in bulk core (0.2-1wt% from cited papers). 44/12 conversion factor since 
...
def startingatmosphere2(Xiktop, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True, differentCO2=True, differentCO=True):
    ...
    def pioxirootscalar(pioxi, ind): # function to find root of (scalar input)
        ...
        return alphaioxi[ind]*pioxiPa**(1/betaioxi[ind])+alphaired[ind]*piredPa**(1/betaired[ind])-Xiktop[ind]*1e6

    # pioxisol = newton(pioxiroot, np.array([2.16850398e-02, 5.58147536e-04, 4.95050415e-05]))
    # print('pioxisol', pioxisol)

    pioxisol = np.array([bisect(pioxirootscalar, 1/GPatobar*1e-1, 1/GPatobar*1e6, args=ind) for ind in irange]) # bisection limits from xlim of Lichtenberg2021 fig. 2. 
...
if foldersaving:
    variablefoldername = ['Pebble - ', 'Planetesimal - '][planetesimalaccretion]+'SolFac='+str(solubilityfactor)+' - IW'+str(deltaIW)+Ppaperlabel[0]+Ppaperlabel[1]+Ppaperlabel[2]+'/'
