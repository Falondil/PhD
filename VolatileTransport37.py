...
Matm = Matmhigh # CHOICE, should it be? 
...
def pTsurf(rsurf, pebblerate=pebblerate, protomass=protomass, Matm=Matm, opacitypho=opacitypho, gamma=atmgamma):
    gsurf = G*protomass/rsurf**2
...
rk, psurf, Tsurf, pressure, temperature, density, D, compressioncalctime = restructuringfromdensity(rk, initialdensity, pebblerate, number_of_structure_boundaries, number_of_structure_iterations, True)
...
def Kifunc(Matm, rsurf=rsurf, protomass=protomass, H2Oindex = 0, meltindex=0): # choice of melt from which experimental K for CO2 is taken
    ...
    KN2 = KH2O # PLACEHOLDER
    
    return np.array([KH2O, KCO2, KN2])
...
Msilvol = Msil+np.sum(Mvol, axis=0) # mass of volatiles + silicate melt inside each layer
...
def startingatmosphere(Matm, Xiktop, AMUi, alphai):
    Ki = Kifunc(Matm)
...
# PLACEHOLDER. Possibly useful. calculates the Ki corresponding to if the non-equilbriated dissolution was in fact an equilibrium
...
Miatmstart = startingatmosphere(Matm, Mvol[:,-1]/Msilvol[-1], amui, alphai)
eqstate = Miatmeqfunc(Miatmstart, amui, alphai, Msil[-1], Mvol[:,-1]) # state of mass conservation and dissolution equilibrium
print(np.sum(eqstate[0])/np.sum(Miatmstart))
...
def tridiagsolver(rk, Mvol0, Msil, Mmet, vmet, D, Pvol, dt=dt): # solves tri-diagonal matrix for volatile motion to neighboring layers of magma ocean via sedimentation and convection (modelled as enhanced diffusion) 
    ...
    rhosil = centeraveraging(Msil/volumefunc(rk)) # calculate layer-specific density of silicate AT layer boundaries
    ...
    diag = np.array([np.append(np.concatenate(([bottomlayerloss[i]], ownlayer[i])), toplayerloss[i]) for i in range(len(volatiles))]) # elements on the diagonal of the matrix A where dM_ik/dt = A M_ik for any volatile i. 
    ...
    for i in range(len(volatiles)):
        ...
        eigval, eigvec = eig(A[i])
        print(np.linalg.cond(eigvec)) # < 1e15 is good
        Cs = np.linalg.solve(eigvec, Mvol0[i])
        Mvol[i,:] = np.real((Cs*np.exp(eigval*dt))@eigvec.transpose())
    return Mvol
        

    # WRONG. We don't want to solve the matrix equation. We know Mvol0 already. Rather we want to find the eigenvalues/vectors s.t. Mvol(t) can be expressed analytically. 
    # sol = np.zeros_like(Mvol)
    # for i in range(len(volatiles)):
    #     diag_band_matrix = np.array([upperdiag[i], diag[i], lowerdiag[i]])
    #     sol[i,:] = solve_banded((1,1), diag_band_matrix, Mvol0[i])
    
    # diag_band_matrix = np.array([upperdiag, diag, lowerdiag])
    # sol = solve_banded((1,1), diag_band_matrix, Mvol0)

    
    # # solve_banded l_and_u = (1,1)
