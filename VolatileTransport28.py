...
initialtemperature, adiabattemperature = surfacetemp+np.zeros(number_of_structure_boundaries), surfacetemp+np.zeros(
...
for j in range(number_of_structure_timesteps):
    ...
    adiabattemperature = topdownadiabat(
        rsvar, drsvar, surfacetemp, pressurelong, Menc)
...
def PCk(p, T, deltaIW=-2.2, Xsulfur=0, Xoxygen=0, NBOperT=2.6, eq2=False):
    if eq2:
...
if constantdensity:
    ...
    Msil, density = massanddensityinterpolation(rk, dr, rsvar, densitylong)
