...
def Kifunc(Matm, fO2, rsurf=rsurf, Tsurf=Tsurf, protomass=protomass, H2Oindex=0, meltindex=0): # choice of melt from which experimental K for CO2 is taken
    ...
    KN2chem = KH2O # PLACEHOLDER, Should be fO2 dependent here
    KN2phys = KH2O # PLACEHOLDER
    KN2 = [KN2chem, KN2phys][N2index]
...
def Miatmeqfunc(Miatm0, fO2, rsurf, Tsurf, AMUi, alphai, Msiltop, Misil0, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at physical+chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        frac = Miatm/AMUi
        Ki = Kifunc(np.sum(Miatm), fO2, rsurf, Tsurf) # calculate the equilibrium constants from pressure (or Matm)
...
eqstate = Miatmeqfunc(Miatmguess, fO2, rsurf, Tsurf, amui, alphai, Msil[-1], Mvol0[:,-1]) # state of mass conservation and dissolution equilibrium
Miatm0 = eqstate[0]
Matm0 = np.sum(Miatm0)
...
for timestep in range(number_of_timesteps):
    ...
    eqstate = Miatmeqfunc(Miatm, fO2, rsurf, Tsurf, amui, alphai, Msil[-1], Mvol[:,-1])
    Miatm = eqstate[0]
