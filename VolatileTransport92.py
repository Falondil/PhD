...
def Miatmeqfunc4(Miatmguess, Miatmextraguess, amui, amuiextra, psurf, Msiltop, Mitop0, differentnitrogen=True, differentCO2=True, differentCO=True, method='bisect'):
    ...
    if method=='newton':
        ...
        def bisectwithlimits(limitfactor, ind):
            uppersign = np.sign(Miatmrootfunc(Miatmguess[ind]*limitfactor, ind))
            lowersign = np.sign(Miatmrootfunc(Miatmguess[ind]/limitfactor, ind))
            if uppersign!=lowersign:
                return bisect(Miatmrootfunc, a=Miatmguess[ind]/limitfactor, b=Miatmguess[ind]*limitfactor, args=(ind, ))
            elif limitfactor < 10: # some stopping point
                return bisectwithlimits(limitfactor+0.05, ind)
            else:
                print('Volatile:', volatiles[ind])
                raise ValueError("Function does not evaluate to opposite signs in the bisection endpoints")
        Miatmeq = np.array([bisectwithlimits(1.01, ind) for ind in irange])
...
for timestep in range(1, number_of_timesteps+1):
    ...
    if timestep % gasspacing == 0 or timestep == number_of_timesteps:
        ...
        # Add accreted volatiles every time step. Not just every time the planet grows and restructures
    ...
    if timestep % restructuringspacing == 0 or timestep == number_of_timesteps:
        ...
        if growthbool and simulatedyears>settime*nosedifrac: # if we want to grow the planet
            ...
            for i in irange: # CHOICE, add late accretion of volatiles
                if latevolatileaccretion and protomass<volatilestoppingmass[i]:
                    Miatm[i] += restructuringspacing*pebblerate*pebblevolatilefrac[i]*dt # add accreted volatile mass to the atmosphere (as oxidized species)
