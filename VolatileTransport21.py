...
RBMnterms = 5 # decide how many positive and negative n terms of the infinite series expansion of 1/sinh to use in the PDF expression. 
relativeconvection = False # use Veestraeten2004 solution but for diffusion variable Mvol/Msil
explicit = True # decide if explicit diffusion scheme is used
evendist = False # decide initial distribution of volatiles to have a constant mass concentration throughout magma ocean
innerdelta = True # decide initial distribution of volatiles to be concentrated in the layer closest to the core
...
plotstepspacing = 50 # Every n timesteps plots are printed
...
structureplotstepspacing = number_of_structure_timesteps
...
pebblerate = 1e-2*1e6 # Pg yr-1, PLACEHOLDER VALUE, x order(s) of magnitude less than what is required to form a 1 Earth mass planet over 5 Myr.
...
def thermalexpansion(pressure): # calculate the volumetric thermal expansion coefficient as a function of pressure, K-1. Abe1997
...
initialtemperature, _ = temperaturefunc3(rs, delta(rs), initialpressure, Menc)  # calculate the initial temperature in each layerboundary, unit K
...
for j in range(number_of_structure_timesteps):
    ...
    temperaturelong, adiabattemperature = temperaturefunctilt(rsvar, drsvar, pressurelong, Menc)
...
def diffusivity(r, pressure, temperature, density, Menc): # calculate diffusivity
    ...
    g = G*Menc/r**2 # calculate local gravity at center of layers
    ...
    Ra = alpha*Cp*density**2*g*deltaT*magmadepth**3/(K*eta) # Rayleigh number
    Nu = (Ra/Rac)**(2/7) # Nusselt number
    return Nu*K/(density*Cp) # km2 year-1

def diffusivitycentering(r, pressure, temperature, density, Menc): # calculate diffusivity when the variables are known on the layer boundaries
    ...
    return diffusivity(rcenter, centerpressure, centertemperature, cdensity, centerMenc)
...
D = diffusivity(layercenters, pressure, temperature, density, menc(Msil)[:-1])
Dmax = np.max(D)
...
for j in range(number_of_timesteps):
    ...
    if relativeconvection: 
        ...
        Mvol = np.array([explicitdiffusion(Mvol[i], Msil, Mmet, np.append(D*density, D[-1]*density[-1]), Pik0[i])+vmassinmet[i] for i in irange])+sedimentation 
