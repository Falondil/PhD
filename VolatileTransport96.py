...
solubilityfactor = 1 # arbitrary factor to multiply all solubilities by. Default is 1. 
deltaIW = -2.0 # CHOICE log scale of difference from oxygen fugacity produced by IW buffer at surface pT.
corevolatileiteration = False # should you adjust the volatile budget to find a volatile core mass consistent with the assumed BSE (H18)? 
homogeneousvolatiles = True # CHOICE. Average the magma ocean concentration across all layers
...
saturation = False # should volatile saturation in metal droplets be limited
...
print('Planetesimal accretion:', planetesimalaccretion)
print('Folder saving:', foldersaving)
...
pebblemetalfrac = 0.324 # mass fraction of metal (iron) in accreted pebbles. Mass fraction here means met/(met+sil)
...
alphaN2chem = 2 # N2(gas) + H2O(gas)+(3-_)*O(melt) = 2 NH_ + 3/2 O(gas) e.g. Grewal2020.
...
# Alternative 1. Old (and incorrect N)
# MH2Otot0 = 2e3*1e-6*earthmass # Fig. 11, AnatomyIII.
# MCO2tot0 = 44/12*((1-coremassfraction)*140*1e-6+coremassfraction*0.3/100)*earthmass # Hirschmann2018 for 140 ppm. Chen2014 for 0.3wt% C in bulk core (0.2-1wt% from cited papers). 44/12 conversion factor since 
# MNtot0 = 10**0.5*1e-6*earthmass # mass fraction of total planet mass of Venus in atmosphere, from same figure in AnatomyIII

# Alternative 2. Only match to planetary reservoirs
Mearthocean = 1.37e21*1e-12 # Pg, AnatomyIII fig 2.

MH2Otot0 = Mearthocean*numEarthoceanscore+770*1e-6*(1-coremassfraction)*earthmass # Huang2026 + Hirschmann2018
MCO2tot0 = 44/12*((1-coremassfraction)*140*1e-6+coremassfraction*0.3/100)*earthmass # Hirschmann2018 for 140 ppm. Chen2014 for 0.3wt% C in bulk core (0.2-1wt% from cited papers). 44/12 conversion factor since 
MNtot0 = (1-coremassfraction)*2.8*1e-6*earthmass # 2.8 ppmw BSE Hirschmann2018 + 0wt% in the core

# Optional: Multiply budget by prefactor to iterate to self-consistent convergent solution
if corevolatileiteration: 
    H2Ofactors = np.array([1038, 574])
    CO2factors = np.array([293, 147])
    Nfactors = np.array([2.6, 2.6])
    MH2Otot0 *= 710**len(H2Ofactors)/np.prod(H2Ofactors) 
    MCO2tot0 *= 140**len(CO2factors)/np.prod(CO2factors) 
    MNtot0 *= 2.8**len(Nfactors)/np.prod(Nfactors) 

# Alternative 3. Take from Venus atmosphere
# sumamuVenus = (0.965*amuCO2+0.035*amuN2) 
# Mvenusnitrogen = Mvenusatm*0.035*amuN2/sumamuVenus # Pg
# MNtot0 = Mvenusnitrogen # assume Venus atmosphere N = Earth total N

# Store all initial volatile masses in volatile mass array
Mitot0 = np.array([MH2Otot0, MCO2tot0, MNtot0])#*stoppingmass # multiply by stoppingmass to get fraction of Earth's volatiles 
...
if latevolatileaccretion:
    ...
    # CHANGED FOR FINAL MASS TO ALSO INCLUDE VOLATILES. 
    pebblevolatilefrac = np.where(volatilestoppingmass>startingmass*earthmass, (Mitot0/initialvolatilefraction-Mitot0)/(volatilestoppingmass-MH2Otot0-MCO2tot0-MNtot0-earthmass*startingmass), 0)
...
XO = 0.05 # 8wt% from Fischer2020 upper limit of core currently. ~5wt% seems like a good guess from Badro2015 fig. 1. 4wt% from Allègre1995
XS = 0.023 # 2.3wt% from Allègre1995
XNi = 0.042/coremassfraction # Hiligren1994 says 4.2mol%. Corresponds to 14% of the 30% metal fraction (same as coremassfraction). 
...
def IWfO2func(p, T): # oxygen fugacity of the IW-buffer Fe + O <-> FeO. Hirschmann2021. Valid for T>1000 K and P between 1e-4 and 100 GPa. "It extrapolates smoothly to higher temperature, though not calibrated above 3000 K." 
    ...
    if cappedtemperature:
        T = np.where(T>3500, 3500, np.where(T>1000, T, 1000)) # K, approximate temperature limit for Hirschmann2021 parametrization
...
def PCfuncTsutsumi(p, T, deltaIW=deltaIWmagmaocean, NBOperT=NBOperT, xhmetal=0): # Tsutsumi2025 
    log10PC = 1.51-0.68*deltaIW-0.33*NBOperT+1.76*np.log10(1-xhmetal)# does not use p or T
    return 10**log10PC*np.ones_like(p)

def PCfunc(p, T, deltaIW=deltaIWmagmaocean, PCpaper=PCpaper): # choice of which PC paper to use
    if PCpaper=='Fischer':
        return PCfuncFischer(p, T, deltaIW)
    elif PCpaper=='Blanchard': 
        return PCfuncBlanchard(p, T, deltaIW)
    elif PCpaper=='Tsutsumi':
        return PCfuncTsutsumi(p, T, deltaIW)
    else:
...
def PNfuncGrewal(p, T, deltaIW=deltaIWmagmaocean, XFeO=XFeOfin, Xsulfur=XS, Xsilicon=0, NBOperT=NBOperT):
    if cappedtemperature:
...
def PNfuncHuang(p, T, deltaIW=deltaIWmagmaocean, Xcarbon=0, Xsulfur=XS, Xsilicon=0, Xoxygen=XO, NBOperT=NBOperT):
    if cappedtemperature:
...
def PNfunc(p, T, deltaIW=deltaIWmagmaocean, PNpaper=PNpaper):
    if PNpaper == 'Grewal': # Grewal2019
        return PNfuncGrewal(p, T, deltaIW) 
    elif PNpaper == 'Huang': # Huang2024
        return PNfuncHuang(p, T, deltaIW)
    else:
        return np.ones_like(p)*10 # AnatomyIII

# Water partition function from Tsutsumi2025. NOT WORKING
...
def Kifunc(Matm, fO2surf, rsurf, protomass, pebblerate, H2Oindex=0, meltindex=2, Miatmfull=None, species='CO2'): # choice of melt from which experimental K for CO2 is taken
    ...
    KH2Oburnham = KH2Ointermediate*1/(np.exp(5.00+np.log(pinbar)*(4.481e-8*Tsurf**2-1.51e-4*Tsurf-1.137)+np.log(pinbar)**2*(1.831e-8*Tsurf**2-4.882e-5*Tsurf+4.656e-2)+7.80e-3*np.log(pinbar)**3-5.012e-4*np.log(pinbar)**4+Tsurf*(4.754e-3-1.621e-6*Tsurf))) # Burnham 1994, eq. 7. Taken to power -1 because their K is defined for the inverse reaction. H2O(melt)+O->2 OH(melt)
...
# From Lichtenberg2021 and referenced changes to a few of the volatiles
def SolubilityParameters(differentCO2=True, differentnitrogen=True, differentCO=True):
    # ppmw/Pa units
    ...
    if differentCO: # eq. 6 or 7 in Yoshioka2018 or eq. 6 & Table 2 "reduced" in Hirschmann2016
    ...
    # ARBITRARY MULTIPLICATION WITH SOLUBILITY FACTOR
    alphaioxi *= solubilityfactor
    alphaired *= solubilityfactor
    return alphaioxi, betaioxi, alphaired, betaired

# WORK MORE
def startingatmosphere2(Xiktop, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True, differentCO2=True, differentCO=True):
    ...
    redmolefactor = xifulltest[3:]/xifulltest[:2] # how many times more moles of the reduced species do we have
    # print('redmolefactor', redmolefactor)

    alphaioxi, betaioxi, alphaired, betaired = SolubilityParameters(differentCO2, differentnitrogen, differentCO)

    def pioxiroot(pioxi): # function to find root of (to solve for pioxi)
        pioxiPa = pioxi*1e9
        piredPa = np.append(pioxiPa[:2]*redmolefactor, 1) # speciation gives ratio of reduced to oxidized volatile species
        return alphaioxi*pioxiPa**(1/betaioxi)+alphaired*piredPa**(1/betaired)-Xiktop*1e6

    def pioxirootscalar(pioxi, ind): # function to find root of (scalar input)
        pioxiPa = pioxi*1e9
        piredPa = pioxiPa*np.append(redmolefactor, 1)[ind] # speciation gives ratio of reduced to oxidized volatile species 
        ret = alphaioxi[ind]*pioxiPa**(1/betaioxi[ind])+alphaired[ind]*piredPa**(1/betaired[ind])-Xiktop[ind]*1e6
        return ret

    # pioxisol = newton(pioxiroot, np.array([2.16850398e-02, 5.58147536e-04, 4.95050415e-05]))
    # print('pioxisol', pioxisol)

    pioxisol = np.array([bisect(pioxirootscalar, 0, 1/GPatobar*1e6, args=ind) for ind in irange]) # bisection limits from xlim of Lichtenberg2021 fig. 2. 
    # print('pioxisol:', pioxisol*GPatobar,' bar')

    # pioxisol = newton(pioxiroot, piguess) # arbitrary starting guess: all pi are equal. Does not typically work at low deltaIW.
    piredsol = pioxisol[:2]*redmolefactor
    pifullsol = np.concatenate((pioxisol, piredsol))
    Matm = 4*pi*rsurf**4*np.sum(pifullsol)/(GPaconversion*G*protomass) # calculate mass of atmosphere
    # print('log10Matm', np.log10(Matm))
    xifull = pifullsol/np.sum(pifullsol)
    Xiatmfull = weightfraction(xifull, amuifull)
    # print('pifullsol: ', pifullsol*GPatobar)
    # print('xifull: ', xifull)
    # print('Xiatmfull: ', Xiatmfull)
    return Xiatmfull*Matm, np.sum(pifullsol), pifullsol

def iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf, protomass, rsurf, psurf, Tsurf, differentnitrogen=True, differentCO2=True, number_of_iterations=100): # computes starting atmosphere until the total mass of each volatile matches the prescribed initial inventory (Mitot0)
    iterXi = np.copy(Xiguess)
    psurfnew = np.copy(psurf)
    for j in range(number_of_iterations):
        Miatmfull, psurfnew, piatmfull = startingatmosphere2(iterXi, amuifull, fO2surf, protomass, rsurf, psurfnew, Tsurf, differentnitrogen, differentCO2)
        Miatmequiv = Miatmfull[:3]+np.append(Miatmfull[-2:]*amuifull[:2]/amuifull[-2:], 0) # reduced species scale up
        Mimelt = iterXi*Mmagma
        # print('Mimelt+Miatmequiv', Mimelt+Miatmequiv)
        factor = np.sqrt(Mitot0/(Mimelt+Miatmequiv))
        # print('factor', factor)
        iterXi *= np.sqrt(factor*1) # geometric mean of factor and 1
    return Miatmfull[:3], Miatmfull[-2:], iterXi/factor, psurfnew, piatmfull
...
def Miatmeqfunc4(Miatmguess, Miatmextraguess, amui, amuiextra, psurf, Msiltop, Mitop0, differentnitrogen=True, differentCO2=True, differentCO=True, method='bisect'):
    Miatmequiv0 = Miatmguess+np.concatenate((Miatmextraguess*amui[:-1]/amuiextra, np.array([0]))) # total equivalent atmospheric volatile mass (tallied as if molecules weighed what the oxidized speciation does) 
    Miatmfullguess = np.concatenate((Miatmguess, Miatmextraguess))

    xifull = molefraction(Miatmfullguess, amuifull) # initial mole fraction of each species
    
    redmolefactor = xifull[3:]/xifull[:2] # how many times more moles of the reduced species do we have
    redmassfactor = Miatmextraguess/Miatmguess[:2] # how much more mass is in reduced speciation

    alphaioxi, betaioxi, alphaired, betaired = SolubilityParameters(differentCO2, differentnitrogen, differentCO)

    sumfrac = np.sum(Miatmguess/amui)+np.sum(Miatmextraguess/amuiextra) # denominator sum for conversion from mass to mole fraction
    
    psurfPa = psurf*1e9 # GPa to Pa

    def Miatmrootfunc(Miatmi, ind):
        pioxi = psurfPa*(Miatmi/amui[ind])/sumfrac
        if ind == 2: # nitrogen has no reduced speciation
            pired = 0
            Miatmired = 0
            Miatmiredequiv = 0
        else:
            pired = pioxi*redmolefactor[ind] # calculate how much the partial pressure of the reduced species is
            Miatmired = Miatmi*redmassfactor[ind] # calculate how much mass is in the reduced species
            Miatmiredequiv = Miatmired*amui[ind]/amuiextra[ind]
        Xi = 1e-6*(alphaioxi[ind]*pioxi**(1/betaioxi[ind])+alphaired[ind]*pired**(1/betaired[ind])) # 1e-6 to convert from ppmw to fraction
        # print(ind, Miatmi, Miatmiredequiv+Miatmi+Xi*Msiltop-Miatmequiv0[ind]-Mitop0[ind])
        return Miatmiredequiv+Miatmi+Xi*Msiltop-Miatmequiv0[ind]-Mitop0[ind]
    
    if method=='newton':
        Miatmeq = np.array([newton(Miatmrootfunc, Miatmguess[ind], args=(ind, )) for ind in irange])
    elif method=='bisect':
        def bisectwithlimits(upperlimitfactor, lowerlimitfactor, ind):
            uppersign = np.sign(Miatmrootfunc(Miatmguess[ind]*upperlimitfactor, ind))
            lowersign = np.sign(Miatmrootfunc(Miatmguess[ind]*lowerlimitfactor, ind))
            if uppersign!=lowersign:
                return bisect(Miatmrootfunc, a=Miatmguess[ind]*lowerlimitfactor, b=Miatmguess[ind]*upperlimitfactor, args=(ind, ))
            elif lowerlimitfactor > 1/10: # some stopping point
                new_upperlimitfactor = upperlimitfactor*1.1 
                new_lowerlimitfactor = 1/new_upperlimitfactor
                return bisectwithlimits(new_upperlimitfactor, new_lowerlimitfactor, ind)
            elif lowerlimitfactor < 1/10 and lowerlimitfactor > 0:
                new_lowerlimitfactor = 0 # test lowest possible bound
                new_upperlimitfactor = 10
                return bisectwithlimits(new_upperlimitfactor, new_lowerlimitfactor, ind)
            else:
                print('Volatile:', volatiles[ind])
                raise ValueError("Function does not evaluate to opposite signs in the bisection endpoints")
        Miatmeq = np.array([bisectwithlimits(1.01, 1/1.01, ind) for ind in irange])
    Miatmextraeq = Miatmeq[:2]*redmassfactor
    # print(redmassfactor)
    
    Miatmredequiveq = Miatmextraeq*amui[:2]/amuiextra
    Miatmeqequiv = Miatmeq+np.append(Miatmredequiveq, 0)
    Mitopeq = Miatmequiv0+Mitop0-Miatmeqequiv 

    # Probably counted wrong since mass of oxygen is already added to silicate layer via reduced species being counted as dissolving as oxidized species.
    oxygenmass = np.sum(16/amuiextra*(Miatmextraguess-Miatmextraeq)) # for each amuiextra units mass change for the reduced species, we have 16 units of oxygen mass change. 
    
    return Miatmeq, Mitopeq, Miatmextraeq, oxygenmass

# Legacy?
def gasmeltfunc(Miatm, Miatmextra, amuifull, Msiltop, Mitop, fO2surf, psurf, Tsurf, NBOperO=NBOperO, XFeO=XFeOfin): 
    ...
    Xnit = Xnitphys+Xnitchem # mass fraction of nitrogen in melt
...
def gasgasfunc(Miatm, Miatmextra, amuifull, fO2, Tsurf, splitbool=True): # Ortenzi2020
    xi0full = molefraction(np.concatenate((Miatm, Miatmextra)), amuifull) # initial mole fraction of each species
...
XH2Oguess = Mitot0[0]/Mmagma
XCguess = Mitot0[1]/Mmagma
XNguess = Mitot0[2]/Mmagma
...
fO2surf0 = fO2func(psurf, Tsurf, deltaIW) 
protomass0 = mc0+Mmagma+Matmguess

Miatmguessold, Xi0old = iterativestartingvolatiles(Mitot0, Matmguess, Mmagma, Xiguess, amui, alphai, fO2surf0, rsurf0, protomass0, pebblerate0, )
# Miatmextraguessold = np.zeros(2) # PLACEHOLDER. guess no H2 or CO at the start. Do via gasgas equilibrium constants instead 
# print('Xi0old', Xi0old)

# Test initial volatiles as function of fO2:
if teststartingvolatiles:
    deltaIWlist = np.linspace(-2, 0, 9)
    fO2surflist = fO2func(psurf, Tsurf, deltaIWlist)

    retarray = []
    for j in range(len(deltaIWlist)):
        ret = iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surflist[j], protomass, rsurf, psurf, Tsurf)
        retarray.append(ret)

    startingvolatilesfig = plt.figure(figsize=(10, 6))
    plt.suptitle('Starting atmosphere as function of oxygen fugacity')
    for i in irange:
        plt.subplot(121)
        plt.semilogy(deltaIWlist, [retarray[j][0][i]/earthmass for j in range(len(deltaIWlist))], '-', color=vcolors[i], label=volatiles[i]) 
        if i != 2:
            plt.semilogy(deltaIWlist, [retarray[j][1][i]/earthmass for j in range(len(deltaIWlist))], '--', color=vcolors[i]) 

        plt.ylabel('Atmosphere mass [M_E]')
        plt.xlabel('deltaIW')
        plt.grid()
        
        plt.subplot(122)
        plt.semilogy(deltaIWlist, [GPatobar*retarray[j][4][i] for j in range(len(deltaIWlist))], '-', color=vcolors[i], label=volatiles[i]) 
        if i != 2:
            plt.semilogy(deltaIWlist, [GPatobar*retarray[j][4][i+3] for j in range(len(deltaIWlist))], '--', color=vcolors[i]) 
    ...
    plt.subplot(122)
    plt.semilogy(deltaIWlist, [GPatobar*sum(retarray[j][4]) for j in range(len(deltaIWlist))], '-', color='k', label='sum')
    plt.legend()

    plt.subplots_adjust(wspace=0.3)  # Increase the horizontal space between subplots


Miatmguess, Miatmextraguess, Xi0, psurf, piatm0, = iterativestartingvolatiles2(Mitot0, Mmagma, Xiguess, amuifull, fO2surf0, protomass, rsurf, psurf, Tsurf)
# print('Xi0', Xi0)
print('piatm0:', piatm0*GPatobar, ' bar at deltaIW = ', deltaIWmagmaocean)
...
# Miatm0, Miatmextra0, oxygenmass = gasgasfunc(Miatm0, Miatmextra0, amuifull, fO2surf0, Tsurf)
# Msil[-1] += oxygenmass
...
PN = PNfunc(pressure, temperature)
...
Dtimearray = np.zeros(number_of_timesteps//savingspacing+2) # create array to fill with diffusivity
...
for timestep in range(1, number_of_timesteps+1):
    ...
    if timestep<1e3: # do first few timesteps extra precise
        dt /= 1e3-timestep # smooth out step size increase
    ...
    if homogeneousvolatiles:
        totalMvol = np.sum(Mvol, axis=1) # sum over all layers
        Mvol = np.array([totalMvol[i]/np.sum(Msil)*Msil for i in irange]) # Mvol,k = C,magmaocean*Msil,k, average concentration times local silicate mass
    ...
    if timestep % gasspacing == 0 or timestep == number_of_timesteps:
        ...
        eqstate = Miatmeqfunc4(Miatm, Miatmextra, amui, amuiextra, psurf, Msil[-1], Mvol[:,-1])
        Miatm = eqstate[0]
        Mvol[:,-1] = eqstate[1] # update magma ocean top layer's volatile content
        Miatmextra = eqstate[2] 
        oxygenmass = eqstate[3] # oxygen mass change from dissolution
        Msil[-1] += oxygenmass # reducing a volatile species lowers the mass. Add the mass to the top layer of the magma ocean. 

        # Miatm, Mvol[:, -1] = gasmeltfunc(Miatm, Miatmextra, amuifull, Msil[-1], Mvol[:,-1], fO2surf, psurf, Tsurf)

        # Add accreted volatiles every gasspacing time step. Not just every time the planet grows and restructures
        for i in irange: # CHOICE, add late accretion of volatiles
            if latevolatileaccretion and protomass<volatilestoppingmass[i] and growthbool and simulatedyears>settime*nosedifrac:
                Miatm[i] += gasspacing*pebblerate*pebblevolatilefrac[i]*dt # add accreted volatile mass to the atmosphere (as oxidized species)

        # gas-gas interaction in atmosphere
        Miatm, Miatmextra, oxygenmass = gasgasfunc(Miatm, Miatmextra, amuifull, fO2surf, Tsurf) # gas-gas speciation 
        Msil[-1] += oxygenmass # reducing a volatile species lowers the mass. Add the mass to the top layer of the magma ocean. 
        Miatmfull = np.concatenate((Miatm, Miatmextra))
    ...
    if timestep % restructuringspacing == 0 or timestep == number_of_timesteps:
        ...
        if not constantdeltaIW:
            deltaIWmagmaocean = deltaIWmassfunc(protomass) # interpolate what deltaIW should be for a given planet mass 
            if homogeneousdeltaIW:
                deltaIW = deltaIWmagmaocean
        fO2surf = fO2func(psurf, Tsurf, deltaIW) # update oxygen fugacity? Wrong?
        
        if PH2Opaper == 'Luo':
            PH2O, CH2Omet = iterativePH2O(pressure, Mvol[0], Msil, Mmet, CH2Ometguess=CH2Omet)
        else:
            PH2O = PH2OTagawa(pressure, temperature, deltaIWmagmaocean)
        PC = PCfunc(pressure, temperature, deltaIWmagmaocean)
        PN = PNfunc(pressure, temperature, deltaIWmagmaocean)
        Pvol = np.array([PH2O, PC, PN]) # array of all the volatile species' partition coefficients (at all magma ocean layers)        
    ...
    if timestep % savingspacing == 1 or timestep == number_of_timesteps:
        ...
        pebbleratetimearray[savingindex] = pebblerate # store pebble accretion rate
        Dtimearray[savingindex] = D # store (enhanced) diffusivity of magma ocean
    ...
    if timestep % plotstepspacing == 1 or timestep == number_of_timesteps:
        ...
        plt.title('Volatile distribution in magma ocean \n'+'Time step '+str(timestep)+', '+"{:.1f}".format(simulatedyears)+' years. '+'Total mass: '+"{:.2E}".format(protomass/earthmass)+' M'+'$_{E}$'+'\n Psurf: '+"{:.1f}".format(psurf*GPatobar)+' bar. Volatile core mass fraction: '+"{:.2E}".format(np.sum(Micore)/mc))
        for i in irange:
    ...
    print('Time step '+str(timestep)+'/'+str(number_of_timesteps)+' completed. t = '+"{:.2f}".format(simulatedyears)+' years. '+'dt = '+"{:.2f}".format(dt)+'. '
          ...
          "{:.2f}".format(100*((protomass-protomass0)/earthmass)/(stoppingmass-startingmass))+'% of mass accreted.')
...
with open('Ppaperfile', 'wb') as f:
    pickle.dump(Ppaperlabel, f)


if foldersaving:
    addonname = ' - corevolatileiteration' if corevolatileiteration else ''
    variablefoldername = ['Pebble - ', 'Planetesimal - '][planetesimalaccretion]+'SolFac='+str(solubilityfactor)+' - IW'+str(deltaIW)+Ppaperlabel[0]+Ppaperlabel[1]+Ppaperlabel[2]+' - '+str(numEarthoceanscore)+' EO in Core'+addonname+'/'
    os.makedirs(variablefoldername, exist_ok=True)
else:
    variablefoldername = ''

# In case program was cut short, remove all elements that didn't get a chance to be filled. 
np.savetxt(variablefoldername+'time.out', timearray[:savingindex], delimiter=',') # 1d file that stores time in years at each time step (0D variable is recorded each time step)
np.savetxt(variablefoldername+'simulationtime.out', simulationtimearray[:savingindex], delimiter=',') # 1d file that stores how much real time has elapsed in running the code (0D variable is recorded each time step)
np.savetxt(variablefoldername+'Surf.out', surfcondtimearray[:, :savingindex], delimiter=',') # 2d file. (rsurf, psurf, Tsurf) order for each saving time step
np.savetxt(variablefoldername+'CMB.out', cmbcondtimearray[:, :savingindex], delimiter=',') # 2d file. (rcmb, pcmb, Tcmb) order for each saving time step

np.savetxt(variablefoldername+'Misurf.out', Miatmtimearray[:, :savingindex], delimiter=',') # 2d file. Miatm for each saving time step
np.savetxt(variablefoldername+'Misurfextra.out', Miatmextratimearray[:, :savingindex], delimiter=',') # 2d file Miatmextra for each saving time step
np.savetxt(variablefoldername+'Micore.out', Micoretimearray[:, :savingindex], delimiter=',') # 2d file. Micore for each saving time step
np.savetxt(variablefoldername+'Mcore.out', Mcoretimearray[:savingindex], delimiter=',') # 1d file. Mcore for each saving time step
np.savetxt(variablefoldername+'Mmagma.out', Mmagmatimearray[:savingindex], delimiter=',') # 1d file. mass of magma ocean for each saving time step
np.savetxt(variablefoldername+'protomass.out', protomasstimearray[:savingindex], delimiter=',') # 1d file. protomass for each saving time step
np.savetxt(variablefoldername+'pebblerate.out', protomasstimearray[:savingindex], delimiter=',') # 1d file. pebblerate for each saving time step
np.savetxt(variablefoldername+'diffusivity.out', protomasstimearray[:savingindex], delimiter=',') # 1d file. (enhanced) diffusivity for each saving time step

np.savetxt(variablefoldername+'timeplot.out', timeplotarray[:plotnumber], delimiter=',') # file that stores time in years at each time step where plots are produced or 1D variables are recorded
np.savetxt(variablefoldername+'rk.out', structureplotarray[:,0][:plotnumber], delimiter=',') # rows are layer boundaries for a given t (corresponding to timeplot.out)
np.savetxt(variablefoldername+'pressure.out', structureplotarray[:,1][:plotnumber], delimiter=',') # rows are pressure profile for a given t (corresponding to timeplot.out)
np.savetxt(variablefoldername+'temperature.out', structureplotarray[:,2][:plotnumber], delimiter=',') # -||- but temperature profile
np.savetxt(variablefoldername+'density.out', structureplotarray[:,3][:plotnumber], delimiter=',') # -||- but density profile
np.savetxt(variablefoldername+'Mtot.out', structureplotarray[:,4][:plotnumber], delimiter=',') # -||- but total mass in each layer
for i in irange: # save a txt-file for each volatiles' mass distribution in the magma ocean
    np.savetxt(variablefoldername+'M'+volatiles[i]+'.out', Mvolplotarray[:plotnumber,i], delimiter=',') # M of the specified volatile in each layer for each plotting time step
    np.savetxt(variablefoldername+'P'+volatiles[i]+'.out', Pvolplotarray[:plotnumber,i], delimiter=',') # partition coefficient of the specified volatile in each layer for each plotting time step
