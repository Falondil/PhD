...
def Kifunc(Matm, fO2surf, rsurf, protomass, pebblerate, H2Oindex=0, meltindex=0, Miatmfull=None, species='CO2'): # choice of melt from which experimental K for CO2 is taken
    ...
    KN2chem = (np.exp(-(183733+172*(Tsurf+273)-5*pinbar)/(R*(Tsurf+273))))**2*fO2surf**(-3/4) # Bernadou2021, eq. 14 & Table 6. fO2 dependence from eq. 18
    KN2phys = np.exp(-(29344+121*(Tsurf+273)+4*pinbar)/(R*(Tsurf+273))) # Bernadou2021, eq. 13 & Table 6.
...
def Miatmeqfunc(Miatmguess, Miatmextra, rsurf, amui, amuiextra, alphai, Msiltop, Misil0, fO2surf, protomass, pebblerate, method='fsolve', fac=0.1, boundsfactor=2): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at physical+chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        ...
        return (Miatm+((Ki[:,0]*frac/(np.sum(frac)+sumfracextra))**(1/alphai[:,0])+(Ki[:,1]*frac/(np.sum(frac)+sumfracextra))**(1/alphai[:,1]))*Msiltop-Mitot0)
