...
Mvenusatm = 4.8e8 # Pg
atmexponent = 1 # PLACEHOLDER CHOICE. what exponent in scaling law for guess of initial atmosphere
...
def restructuringfromdensity(r, rho, Matm, mc, pebblerate, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, Miatm=None, species='CO2', plotting = False):
...
def restructuring(r, M, Matm, mc, pebblerate, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, Miatm=None, species='CO2', plotting=False):
    starttime = time.time()
...
XFeO = 0.1 # PLACEHOLDER. NEEDED FOR PNfunc. GET CORRECT CALCULATION OF THIS. 
...
def fO2func(p, T, deltaIW=0, cappedfraction=False): # oxygen fugacity 
...
def PCfunc(p, T, deltaIW=0, Xoxygen=0, Xsulfur=0, NBOperT=2.6, microprobe=False):
...
def Kifunc(Matm, fO2surf, rsurf, protomass, pebblerate, H2Oindex=0, meltindex=0, Miatm=None, species='CO2'): # choice of melt from which experimental K for CO2 is taken
...
def startingatmosphere(Matm, Xiktop, AMUi, alphai, fO2surf, rsurf, protomass, pebblerate):
...
def Miatmeqfunc(Miatmguess, rsurf, AMUi, alphai, Msiltop, Misil0, fO2surf, protomass, pebblerate, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
...
def gasgasfunc(Miatm, Miatmextra, amui, amuiextra, fO2, Tsurf, splitbool=True):
    # Matm = np.sum(Miatm)+np.sum(Miatmextra) # WRONG
    
    amuifull = np.concatenate((amui, amuiextra)) # combine atomic mass into single array
    ...
    Kwat, Kcar = Kigasgas(Tsurf, fO2) # equilibrium constants for gas-gas reactions
    xH2O = xwat/(1+1/(Kwat*fO2)**(1/2))
    xCO2 = xcar/(1+1/(Kcar*fO2**1/2))
    xiextra = np.array([xwat-xH2O, xcar-xCO2])
    xN2 = 1-np.sum(xiextra)-xH2O-xCO2
    ...
    if splitbool:
        return np.split(Miatmfull, [len(Miatm)])
    else:
        return Miatmfull, xifull, wifull, Miatm[2]

def Kigasgas(Tsurf, fO2):
    R = 8.31446261815324 # J/(mol*K), universal gas constant
    RT = R*Tsurf
    # Ortenzi2020. coefficients for delta G for oxygenic formation of CO, CO2 and H2O
    aH2O, bH2O, cH2O = -483095.0, 25.3687, 21.9563
    aCO, bCO, cCO = -214104.0, 25.2183, -262.1545 # units 1/T, unitless, 1/T
    aCO2, bCO2, cCO2 = -392647.0, 4.5855, -16.9762
    def fit(a, b, c):
        return a + b*Tsurf*np.log(Tsurf) + c*Tsurf
    Gwat = 2*fit(aH2O, bH2O, cH2O)
    Gcar = fit(aCO2, bCO2, cCO2)-fit(aCO, bCO, cCO)
...
#------------------------Add volatiles to magma ocean--------------------------
# initial concentration of volatiles in each layer
Mmagma = np.sum(Msil) # mass of entire magma ocean to spread out my volatile masses inside
XH2O0 = MH2Otot0/Mmagma
XC0 = MCtot0/Mmagma
XN0 = MNtot0/Mmagma
Xi0 = np.array([XH2O0, XC0, XN0]) # mass fraction inside each layer at the start of the simulation

# initial volatile masses in magma ocean and core
Mvol0 = np.array([XH2O0*Msil, XC0*Msil, XN0*Msil])
...
# Initialize atmosphere volatiles
fO2surf0 = fO2func(pressure[-1], temperature[-1]) 
protomass0 = mc0+np.sum(Micore0)+np.sum(Mtot0)+Matmguess
Miatmguess = startingatmosphere(Matmguess, Mvol0[:,-1]/Msilvol[-1], amui, alphai, fO2surf0, rsurf0, protomass0, pebblerate0)
eqstate = Miatmeqfunc(Miatmguess, rsurf, amui, alphai, Msil[-1], Mvol0[:,-1], fO2surf0, protomass0, pebblerate0) # state of mass conservation and dissolution equilibrium
Miatm0 = eqstate[0]
Miatmextra0 = np.zeros(2) # Guess no H2 or CO at the start. 
print(np.sum(Miatm0)/np.sum(Miatmguess))
protomass0 = mc0+np.sum(Micore0)+np.sum(Mtot0)+np.sum(Miatm0) # recalculate using correct atmosphere mass
...
if continuefromrun: # load variables from previous simulation end step
    with open('picklefile', 'rb') as f:
        simulatedyears, protomass0, pebblerate0, mc0, Msil, Mmet, rsurf, rk, rcore, pressure, temperature, density, D, CH2Omet, Pvol, Mvol, Micore0, Miatm, Miatmextra0, fO2surf0, eigval0, eigvec0, Vinv0 = pickle.load(f)
...
for timestep in range(number_of_timesteps):
    ...
    if timestep % plotstepspacing == 0 or timestep == number_of_timesteps-1:
        ...
        plt.title('Volatile distribution in magma ocean \n'+'Time step '+str(timestep+1)+', '+"{:.1f}".format(simulatedyears)+' years. '+'Total mass: '+"{:.2E}".format(protomass/earthmass)+r"$\ M_{\mathrm{E}}$"+'\n'+r"$M_{\mathrm{atm}} = $"+"{:.1f}".format(Matm/np.sum(Miatm0))+r"$\ M_{\mathrm{atm}, 0}$"+'\n Volatile core mass fraction: '+"{:.2E}".format(np.sum(Micore)/mc))
        for i in irange:
            ...
            plt.axis((None, None, min(Xi0)*1e-4, max(Xi0)*2)) 
            plt.grid()
    ...
    print('Time step '+str(timestep+1)+'/'+str(number_of_timesteps)+' completed. t = '+"{:.2f}".format(simulatedyears)+' years. '+
          ...
          "{:.2f}".format(100*((protomass+np.sum(Miatm))/earthmass-protomass0/earthmass)/(stoppingmass-massscaling))+'% of mass accreted.')
    ...
    print('Simulation speed = '+str(currentsimulationspeed)+' yr/s. '+'Time step speed = '+str(timestep/currentlooptimeelapsed)+' s-1')
...
continuelist = [simulatedyears, protomass, pebblerate, mc, Msil, Mmet,
                rsurf, rk, rcore, pressure, temperature, density, D, CH2Omet, Pvol,
