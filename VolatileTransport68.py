...
# CHOICES
cappedtemperature = False
constanttemperature = False
continuefromrun = False # are you continuing from an earlier run?
nosedifrac = 1e-5 # for how big fraction of the total time should sedimentation be halted. CHOICE 0 to 1. 
...
def PCfunc(p, T, deltaIW=deltaIW, Xoxygen=0, Xsulfur=0, NBOperT=NBOperT, microprobe=False):
    if cappedtemperature:
...
def PNfunc(p, T, XFeO=XFeO,  Xsulfur=0, Xsilicon=0, NBOperT=NBOperT):
    if cappedtemperature:
...
def Kifunc(Matm, fO2surf, rsurf, protomass, pebblerate, H2Oindex=0, meltindex=0, Miatmfull=None, species='CO2'): # choice of melt from which experimental K for CO2 is taken
    ...
    if cappedtemperature:
        Tsurf = np.where(Tsurf>1300, 1300, Tsurf) # approximate limit of Bernadou2021
...
def startingatmosphere(Matm, Xiktop, AMUi, alphai, fO2surf, rsurf, protomass, pebblerate):
    ...
    frac = Xiktop**alphai[:,1]*AMUi/Kiused
...
def Miatmeqfunc(Miatmguess, Miatmextra, rsurf, AMUi, AMUiextra, alphai, Msiltop, Misil0, fO2surf, protomass, pebblerate, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at physical+chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        ...
        return (Miatm+((Ki[:,0]*frac/(np.sum(frac)+np.sum(fracextra)))**(1/alphai[:,0])+(Ki[:,1]*frac/(np.sum(frac)+np.sum(fracextra)))**(1/alphai[:,1]))*Msiltop-Mitot0)
    ...
    if method=='fsolve': # appears to be stably solving further-from-equilibrium starting guesses
        Miatmeq = fsolve(Miatmrootfunc, Miatmguess, xtol=1e-10) # find roots to the function with a starting guess being the unequilibrated atmospheric mass of the volatile species.
    else:
...
def gasgasmelt(Miatm, Miatmextra, Miktop, Msiltop, amui, amuiextra, psurf, Tsurf, fO2surf, H2Oindex=0, meltindex=0, method='fsolve'): # handles both dissolution (gas-melt reaction) and gas-gas speciation simultaneously
    ...
    if method=='fsolve': # appears to be stably solving further-from-equilibrium starting guesses
        Miatmeq = fsolve(Miatmrootfunc, Miatm, xtol=1e-10) # find roots to the function with a starting guess being the unequilibrated atmospheric mass of the volatile species.
    else:
        Miatmeq = broyden1(Miatmrootfunc, Miatm, f_tol=1e-8) # use Broyden's good method instead
    print(str(np.isclose(Miatmrootfunc(Miatmeq), np.zeros_like(Miatmeq)))+'. Miatm,eq/Miatm0: '+str(Miatmeq/Miatm)) # check if roots are valid
...
for timestep in range(1, number_of_timesteps+1):
    ...
    print('Time step '+str(timestep)+'/'+str(number_of_timesteps)+' completed. t = '+"{:.2f}".format(simulatedyears)+' years. '+'dt = '+"{:.2f}".format(dt)+'. '
          "{:.2f}".format(100*(timestep)/number_of_timesteps)+'% of time steps done, '+
          "{:.2f}".format(100*((protomass+Matm-protomass0)/earthmass)/(stoppingmass-massscaling))+'% of mass accreted.')
    ...
    timeelapsedstr = str(currentlooptimeelapsed//86400)
