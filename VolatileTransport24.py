...
def topdownadiabat(rvar, drvar, surfacetemp, pressure, Menc): # Layer step-wise calculating the adiabatic temperature profile
    ...
    for i in range(1, len(rvar)):
        ...
        adiabattemperatureflip[i] = adiabattemperatureflip[i-1]*np.exp(-drflip*alphaflip*G*Mencflip/(Cp*rflip**2)) # take a layer step in temperature
...
def bottomupadiabat(rvar, drvar, coretemp, pressure, Menc): # Layer step-wise calculating the adiabatic temperature profile
    ...
    for i in range(1, len(rvar)):
        ...
        adiabattemperature[i] = adiabattemperature[i-1]*np.exp(-drvar*alpha*G*Menc/(Cp*rvar**2)) # take a layer step in temperature
...
def Rayleigh(r, pressure, temperature, density, Menc, deltaTad, convectivedepth): # calculate Rayleigh number
...
def diffusivity(r, pressure, temperature, density, Menc, deltaTad, convectivedepth): # calculate diffusivity
...
D = diffusivity(layercenters, pressure, temperature, density, centeraveraging(menc(Msil)))
Dmax = np.max(D)
