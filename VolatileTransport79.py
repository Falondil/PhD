...
MCtot0 = 12/44*(526*1e-6+coremassfraction*0.3/100)*earthmass # Alt 2. Marty2012 for 526 ppm. Chen2014 for 0.3wt% C in bulk core (0.2-1wt% from cited papers). 12/44 conversion factor since we count mass of carbon in terms of CO2.
...
def IWfO2func(p, T): # oxygen fugacity of the IW-buffer Fe + O <-> FeO. Hirschmann2021. Valid for T>1000 K and P between 1e-4 and 100 GPa. "It extrapolates smoothly to higher temperature, though not calibrated above 3000 K." 
    ...
    if cappedtemperature:
        T = np.where(temperature>3500, 3500, np.where(temperature>1000, temperature, 1000)) # K, approximate temperature limit for Hirschmann2021 parametrization
...
def PCfunc(p, T, deltaIW=deltaIWmagmaocean, Xoxygen=XO, Xsulfur=XS, NBOperT=NBOperT, microprobe=False):
    if cappedtemperature:
