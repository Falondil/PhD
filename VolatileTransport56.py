...
atmgamma = triatomicgamma # CHOICE. adiabatic gamma from the top of the outgassed atmosphere (bottom of the envelope) up to the RCB. 
...
def pTrcb(rsurf, pebblerate=pebblerate0, protomass=protomass, gamma=atmgamma, a=a0, Lstar=Lsun, beta=2):
...
def pTenvbot(rsurf, pebblerate=pebblerate0, protomass=protomass, gamma=atmgamma, a=a0, Lstar=Lsun, beta=2):
    mgas = 2.2*mproton
...
def Tatmintegral(psurf, penvbot, Tenvbot, species='CO2'): # integrates the temperature from the bottom of the envelope to the surface of the planet
    dT = 1 # K, choice
...
def pTsurf(rsurf, Matm=Matmguess, pebblerate=pebblerate0, protomass=protomass, gamma=atmgamma, a=a0, Lstar=Lsun, species='CO2'):
    gsurf = G*protomass/rsurf**2
    Ltot = pebblerate*G*protomass/rsurf
    ...
    Tatmbot = Tatmintegral(psurf, penv, Tenv, species)
...
def nabla_ad_atm(temperature, species='CO2'): 
    ...
    return Rspecific/Cp # nabla_ad. unitless
...
def restructuringfromdensity(r, rho, Matm, mc, pebblerate = pebblerate0, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, plotting = False):
...
def restructuring(r, M, Matm, mc, pebblerate = pebblerate0, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, plotting=False):
    starttime = time.time()
    ...
    psurf, Tsurf = pTsurf(rvar[-1], Matm, pebblerate, protomass, a=a) # calculate pressure and temperature at the surface of the world
...
rk, psurf, Tsurf, pressure, temperature, density, D, compressioncalctime = restructuringfromdensity(rk, initialdensity, Matmguess, mc0, pebblerate0, number_of_structure_boundaries, number_of_structure_iterations, True)
...
def Kifunc(Matm, fO2surf, rsurf=rsurf, protomass=protomass, pebblerate=pebblerate0, H2Oindex=0, meltindex=0): # choice of melt from which experimental K for CO2 is taken
    psurf, Tsurf = pTsurf(rsurf, Matm=Matm, protomass=protomass, pebblerate=pebblerate, a=a)
...
def Miatmeqfunc(Miatmguess, rsurf, AMUi, alphai, Msiltop, Misil0, fO2surf, protomass=protomass, pebblerate=pebblerate0, method='fsolve'): # function to find the dissolution equilbrium solution of the mass of volatile species in the atmosphere 
    ...
    def Miatmrootfunc(Miatm): # function of Miatm that evaluates to 0 at physical+chemical dissolution equilbrium between top layer of magma ocean and the atmosphere
        frac = Miatm/AMUi
        Ki = Kifunc(np.sum(Miatm), fO2surf, rsurf, protomass, pebblerate) # calculate the equilibrium constants from pressure (or Matm)
...
for timestep in range(number_of_timesteps):
    ...
    # LEGACY. # total mass of planet (protomass and pebblerate) updating used to be here, outside of either stepping boolean check. If errors, MOVE BACK HERE  
    ...
    print('Time step '+str(timestep+1)+'/'+str(number_of_timesteps)+' completed. t = '+"{:.2f}".format(simulatedyears)+' years. '+
          ...
          "{:.2f}".format(100*(protomass/earthmass-massscaling)/(stoppingmass-massscaling))+'% of mass accreted.')
    checktime = time.time()
...
print('Time elapsed in looping: '+str(looptimeelapsed)+'. Simulation speed: '+str(simulationspeed))
