...
IWpaper = ['Komabayashi', 'Hirschmann'][1] # paper source for KIW
FMQpaper = ['Deng', 'Girani'][0] # paper source for K_Fe3+
...
def iron_wustite_K_Hirschmann(P, T): # log10 fO2 = DeltaIW - 2log10(KIW) --> log10 fO2_IW = -2log10(KIW) 
...
def Fe3plusreact_K_Deng(P, T, Vtestplot=Vtestplot, rootsolver='brentq'): 
    # ----------Deng 2020---------- 
    # Up to 140 GPa, 4000 K
    def VDeng(P, component): # calculates V_Mg14Fe2Si16O49 or V_Mg14Fe2Si16O48 by inverting fourth order Birch-Murnaghan equation of state for P(V,T0) at T0 = 3000 K and adding thermal expansion term B_TH (Eq. 2, Deng 2020)
        ...
        def P3000rootfunc(V): # calculates V of assumed melt molecules in Deng 2020 (Eq. 6 & 7) then subtracts P
        ...
        if rootsolver=='fsolve':
            Vguess = 0.95*V0 # starting guess. Expect slightly smaller after bulk compression + thermal expansion
            V_P_T, info, ier, mesg  = fsolve(P3000rootfunc, Vguess, xtol=1e-10, full_output=True, maxfev=400, factor=fac) # roots (V(P) at 3000 K) from fourth order Birch-Murnaghan equation of state
            if True:
                print(
                    f"P={P:.4f}, "
                    f"V/V0={V_P_T[0]/V0:.4f}, "
                    f"nfev={info['nfev']}, "
                    f"message={mesg}"
                )

        elif rootsolver=='brentq':
            # interval to check for a minimum of P3000rootfunc
            Vlo = V0*0.3 
            Vhi = V0*1.5 

            P3000min = minimize_scalar(P3000rootfunc, bounds=(Vlo, Vhi), method='bounded')
            Vmin = P3000min.x # where is a turnaround found, if any exists. Will be Vhi iff P3000rootfunc is monotonically decreasing as is physical
            Fmin = P3000min.fun
            if Fmin>0:
                V_P_T = Vmin # best option remaining
                print(f'WARNING: No physical root found in supplied V interval for this P={P:.2f} GPa, T={T:.2f} K')
            else:
                V_P_T = brentq(P3000rootfunc, Vlo, Vmin, xtol=1e-10)

            # print('V_P_T/V0', V_P_T/V0)
            # print(P, P3000rootfunc(Vlo), P3000rootfunc(Vmin)) # Check for opposite sign

        else:
            raise ValueError(f'Specified root finder method {rootsolver} does not exist')
    ...
    if Vtestplot:
        ...
        plt.xlabel('Pressure [GPa]')
        plt.ylabel(f'Molar volume [Å$^{3}$ molecule$^{-1}$] at T = {T:.4g} K')
        plt.legend()
        plt.xlim(left=P0)
        plt.ylim(top=1400)
        ...
        plt.xlabel('Pressure [GPa]')
        plt.ylabel(f'Molar volume [cm$^{3}$ mole$^{-1}$] at T = {T:.1g} K')
        plt.xlim(left=P0)
        plt.ylim(top=12)
    ...
    # print(T, integral_error_ox/integralVdP_ox, integral_error_red/integralVdP_red, integral_error_ox/integralDeltaVdP, integral_error_red/integralDeltaVdP)
...
def Fe3plusreact_K_Girani(P,T):
    # Fill in this
    return 


def Fe3plusreact_K(P, T, paperchoice=FMQpaper):
    if paperchoice == 'Deng':
        return Fe3plusreact_K_Deng(P,T)
    elif paperchoice == 'Girani':
        return Fe3plusreact_K_Girani(P,T)
    else:
        raise ValueError('Invalid paper choice for Fe3+ reaction')
...
def n_to_activity(nFe, nFeO, nFeO15, OperFe=1, xO=xFeO, method=2):
    ...
    if method == 0:
        ...
        # Alternative 2: Use activity coefficients (a_i = gamma_i x_i)
            ...
            # print('activityratio', activityratio) # 0.86 at 1682 K, infinite dilution
...
# nlayers = 50 # CHOICE
# Plin = np.linspace(1, 100, nlayers)[::-1]
# Tlin = np.linspace(3000, 4000, nlayers)[::-1] 
...
# %%% 
# O/Fe plot for a few (P, T)
linestylearray = [':','-.','--','-']
speciescolors = ['tab:blue', 'tab:orange', 'tab:green'] # Fe, FeO, FeO1.5 in that order
Ptestarray = np.array([0, 25, 50, 75], dtype='float') 
Ttestarray = np.interp(Ptestarray, Plin[::-1], Tlin[::-1])

print(Ptestarray, Ttestarray)

SossiTest = True
if SossiTest:
    Ptestarray[0] = 0.0001 # 1 bar in GPa
    Ttestarray[0] = 2173 # K 
...
if OperFe_zoom or SossiTest:
    OperFe_min = 0.90
    OperFe_max = 1.30
    s = 40
    # OperFe_array = np.logspace(np.log10(OperFe_min), np.log10(OperFe_max), s) # O/Fe, ratio of number of atoms
else:
...
# Plot request from Sossi, mole specation w.r.t. DeltaIW at (P, T) = (1 bar, 2173 K)
if SossiTest:
    DeltaIW_Mine = DeltaIW_array[0] # save
    Fe3plusperFeT_Mine = equilibrium_compositions_2[0, 2, :] # Fe3+/FeT at 1 bar, 2173 K w.r.t O/Fe
    # plt.figure()
    # for j1 in range(1):
    #     for j2 in range(nspecies):
    #         for j3 in range(s):
    #             plt.plot(DeltaIW_array[j1], equilibrium_compositions_2[j1, j2], color=speciescolors[j2], linestyle='-')
    # species_legend = plt.legend(handles=species_handles, title='Species', bbox_to_anchor=(1, 1), loc='upper left')
    # plt.xlim(left=-2.3, right=+4) # arbitrary
...
plt.figure()
for j1 in range(1, len(Ptestarray)):
...
PT_legend = plt.legend(handles=PT_handles[1:], title='Assumed CMB\nconditions')
...
plt.xlabel('time [years]')
plt.ylabel('$\Delta$IW at the magma ocean surface\nwith (P, T) = ('f'{(Ptestarray[0]*1e9*1e-5):.0f} bar'+', '+f'{Ttestarray[0]:.0f}'+' K)')
plt.title('Fe sedimenting from the magma ocean')
PT_legend = plt.legend(handles=PT_handles[1:], title='Assumed CMB\nconditions', loc=(0.01,0.55))
...
# and now for just P=75 GPa case
plt.figure()
plt.semilogx(t_matrix[-1][:-1], DeltaIW_matrix[-1], color='k', linestyle=linestylearray[j1])
plt.xlabel('time [years]')
plt.ylabel('$\Delta$IW at the magma ocean surface\nwith (P, T) = ('f'{(Ptestarray[0]*1e9*1e-5):.0f} bar'+', '+f'{Ttestarray[0]:.0f}'+' K)')
plt.title('Fe sedimenting from the magma ocean, P=75 GPa case')
...
minlogKdifference = min(logKdifference) # useful for plotting

plt.figure()
plt.plot(Tarray, logKdifference, color='k')
plt.title("$\Delta$IW-$\Delta$Fe$^{3+}$ at P = 1 bar")
plt.xlabel('Temperature [K]')
plt.ylabel('2 log$_{10}$(K$_{\mathrm{IW}}) - $4 log$_{10}$(K$_{\mathrm{Fe}^{3+}}$) [dex]')

logKdifference3000 = np.interp(Ttestarray[0], Tarray, logKdifference) # what is DeltaIW - DeltaFMQ at our surface temperature 
logKdifference2173 = np.interp(2173, Tarray, logKdifference) # what is DeltaIW - DeltaFMQ at 2173 K
plt.vlines(3000, ymin=minlogKdifference, ymax=logKdifference3000, color='k', linestyle=':', alpha=0.5) # draw dotted line up to intercept at T = 3000 K
plt.vlines(2173, ymin=minlogKdifference, ymax=logKdifference2173, color='k', linestyle=':', alpha=0.5) # draw dotted line up to intercept at T = 2173 K
plt.hlines(logKdifference3000, xmin=Tarray[0], xmax=3000, color='k', linestyle=':', alpha=0.5)
plt.hlines(logKdifference2173, xmin=Tarray[0], xmax=2173, color='k', linestyle=':', alpha=0.5)

# plt.hlines(logKdifference3000, xmin=3750, xmax=4000, color='k')
# plt.hlines(logKdifference2173, xmin=3750, xmax=4000, color='k')
# plt.vlines((3750+4000)/2, ymin=logKdifference3000, ymax=logKdifference2173, color='k')
plt.xlim(Tarray[0], Tarray[-1])
plt.ylim(minlogKdifference, int(max(logKdifference)+1))
# plt.fill_between(Tarray, logKdifference2173, logKdifference3000,color='k',alpha=0.2)
...
Fe3plusperFe2plus_lin_Sossi_uncorrected = 10**(0.252*DeltaIWwrtFe3plus-1.53) # Sossi2020 corrected regression in Hirschmann2022 (Eq. 34)

Fe3plus_lin_Sossi = Fe3plusperFe2plus_lin_Sossi/(1+Fe3plusperFe2plus_lin_Sossi) # backwards conversion 
Fe3plus_lin_Sossi_uncorrected = Fe3plusperFe2plus_lin_Sossi_uncorrected/(1+Fe3plusperFe2plus_lin_Sossi_uncorrected) # backwards conversion 
...
# My calc.
Fe3plusperFe2plus_activity_Mine = 0.86*Fe3plusperFeT_Mine*(1-Fe3plusperFeT_Mine)**(-1)
Fe3plusperFeT_activity_Mine = Fe3plusperFe2plus_activity_Mine/(1+Fe3plusperFe2plus_activity_Mine)
DeltaIW_Mine_Calc = 4*np.log10(Fe3plusperFe2plus_activity_Mine)-2*np.log10(KFMQ_surf_test**2/KIW_surf_test)
...
plt.plot(DeltaIWwrtFe3plus, Fe3plus_lin_Sossi_uncorrected, linestyle='--', linewidth=2, color='r', label='Sossi2020, raw')
plt.plot(DeltaIW_Sossi_data, Fe3plusperFeT_Sossi_data, 'o', color='k', mfc='white', markersize=10, label='Sossi2020 data')
plt.plot(DeltaIW_Mine_Calc, Fe3plusperFeT_activity_Mine, '-', label='Calculated equilibrium', color='k') # plot my own solution w.r.t. DeltaIW, by request of Sossi
plt.plot(DeltaIW_Mine, Fe3plusperFeT_activity_Mine, 'x', label='Equilibrium points', color='k') # plot my own solution w.r.t. DeltaIW, by request of Sossi
