...
deltaT = 2000 # temperature above that of an adiabat, K, PLACEHOLDER
...
for j in range(number_of_structure_timesteps):
    ...
    rsvar = newlayerboundaries(Msil, density)
    ...
    drsvar = delta(rsvar)
    pressure = pressurefunc2(rsvar, drsvar, density, Menc)
    temperature = temperaturefunc(rsvar, drsvar, pressure, Menc)
    ...
    if j%plotstepspacing==0 and compressionplotting:      
        ...
        ax3.plot(rsvar, Menc, color=colors[1])
...
# masses
# Msil = silicatedensity*layervolumes # Alt 0. No compression
Msil, _ = massanddensityinterpolation(rk, dr, rsvar, density) # Alt 1. Mass interpolated from compressed densities
Msilplot = np.copy(Msil) # copy the "initial" post-compression mass for plotting purposes 
...
for j in range(number_of_timesteps):
    ...
    if j%plotstepspacing==0: 
        ...
        ax1.set(xlabel='Distance from planet center [km]', ylabel=('Silicate mass [Pg]'), aspect=3/4*(rsurf-rc)/max(np.concatenate((Msil,Msilplot))))
        ax1.plot(layercenters, Msilplot, '-.', color='k', label='initial silicate mass')
