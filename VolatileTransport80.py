...
stoppingmass = 0.7 # after how many earth masses do we stop accretion
...
def PCfuncold(p, T, deltaIW=deltaIWmagmaocean, Xoxygen=XO, Xsulfur=XS, NBOperT=NBOperT, microprobe=False):
    if cappedtemperature:
...
def PCfunc(p, T, deltaIW=deltaIWmagmaocean, valence=2): # Blanchard2022
    if cappedtemperature:
...
# LEGACY # Nitrogen partition function. Parametrization from Grewal2019. Pressure, temperature in GPa, K units.
def PNfuncold(p, T, XFeO=XFeO, Xsulfur=XS, Xsilicon=0, NBOperT=NBOperT):
    if cappedtemperature:
...
def PNfunc(p, T, Xcarbon=0, Xsulfur=XS, Xsilicon=0, Xoxygen=XO, NBOperT=NBOperT, deltaIW=deltaIWmagmaocean):
    if cappedtemperature:
