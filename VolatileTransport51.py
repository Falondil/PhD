...
max_number_of_plots = 400 # CHOICE
...
massscaling = 1 # how many earth masses to start with. >0.3 seems very safe (deltaTad = negligible), >0.1 might be safe (temperature profile looks plausible).
...
pebblemetalfrac = 0.3  # mass fraction of metal (iron) in accreted pebbles PLACEHOLDER
...
atmgamma = triatomicgamma # CHOICE
...
def restructuringfromdensity(r, rho, Matm, mc, pebblerate = pebblerate, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, plotting = False):
    M = rho*volumefunc(r)  # calculates initial layer mass
    return restructuring(r, M, Matm, mc, pebblerate, number_of_structure_boundaries, iterations, plotting)
    

# calculates p, T, rho profile and Ra when the layermasses are known 
def restructuring(r, M, Matm, mc, pebblerate = pebblerate, number_of_structure_boundaries = number_of_structure_boundaries, iterations = number_of_structure_iterations, plotting=False):
    starttime = time.time()
    ...
    print('M: '+str(np.sum(M))) # PLACEHOLDER
    print('layermasses: '+ str(np.sum(layermasses))) # PLACEHOLDER
    ...
    print('massshort: '+str(np.sum(massshort))) # PLACEHOLDER
    print('Rcmb:'+str(rshort[0])+', Rsurf: '+str(rshort[-1])) # PLACEHOLDER
...
rk, psurf, Tsurf, pressure, temperature, density, D, compressioncalctime = restructuringfromdensity(rk, initialdensity, Matmguess, mc0, pebblerate, number_of_structure_boundaries, number_of_structure_iterations, True)
...
# Mvol, conv, sedi, Micore, corediff = tridiagsolver(rk, Mvol0, Msil, Mmet, vmet, D, Pvol, Micore0, dt)
# plt.figure()
# for i in irange:
...
# %% Transport loop
timearray = np.zeros(number_of_timesteps) # create array to match time step with years simulated
Micoretimearray = np.zeros((len(volatiles), number_of_timesteps)) # create array to fill with volatile mass in the core
Miatmtimearray = np.zeros((len(volatiles), number_of_timesteps)) # create array to fill with volatile mass in the atmosphere
surfcondtimearray = np.zeros((3, number_of_timesteps)) # create array to fill with surface conditions
cmbcondtimearray = np.zeros((3, number_of_timesteps)) # create array to fill with conditions at the core mantle boundary 
...
Mvolplotarray = np.zeros((number_of_timesteps//plotstepspacing+2, len(volatiles), number_of_layers))
...
for timestep in range(number_of_timesteps):
    ...
    timearray[timestep] = simulatedyears

    # volatile motion inside magma ocean
    Mvol, conv, sedi, Micore, corediff = tridiagsolver(rk, Mvol, Msil, Mmet, vmet, D, Pvol, Micore, dt)
    Mtot = Msil+Mmet+np.sum(Mvol, axis=0)
    mc = mc+np.sum(corediff) # add volatile mass to core mass
    Micoretimearray[:, timestep] = Micore # store volatile mass in core for each time step 
    ...
    Miatmtimearray[:, timestep] = Miatm # store volatile mass in atmosphere for each time step
    
    surfcondtimearray[:, timestep] = np.array([rk[-1], psurf, Tsurf])
    cmbcondtimearray[:, timestep] = np.array([density[0], pressure[0], temperature[0]])
    ...
    if timestep % restructuringspacing == 0 or timestep == number_of_timesteps-1:
        ...
        rk, psurf, Tsurf, pressure, temperature, density, D, compressioncalctime = restructuring(rk, Msil, Matm, mc, pebblerate, number_of_structure_boundaries, 1, False)
        dr = rk[1]-rk[0] # same layerwidth for all layers
        rsurf = rk[-1]  # new surface is the compressed surface
        layercenters = centeraveraging(rk)
        layervolumes = volumefunc(rk) # calculate volume of each magma ocean layer 
    ...
    if timestep % plotstepspacing == 0 or timestep == number_of_timesteps-1:
        ...
        fig.suptitle('Time step '+str(timestep+1)+', '+"{:.1f}".format(simulatedyears)+' years. '+r"$M_{\mathrm{atm}} = $"+"{:.1f}".format(Matm/Matm0)+r"$\ M_{\mathrm{atm}, 0}$"+'\n Volatile core mass fraction: '+"{:.2E}".format(np.sum(Micore)/mc)+', change: '+"{:.2E}".format(np.sum(corediff)/mc), y=1.10)
        for i in irange:
        ...
        fig.savefig('Plots/VideoPlots/fig'+plotstring+'.png', bbox_inches='tight')
        ...
        ax2.semilogy(pressure, np.transpose(Pvol), label=volatiles)
        ax2.set_xlabel('Pressure [GPa]')
        fig.savefig('Plots/StructurePlots/PartFunc'+plotstring+'.png', bbox_inches='tight')        
        
    print('Time step '+str(timestep+1)+'/'+str(number_of_timesteps)+' completed. t = '+"{:.1f}".format(simulatedyears)+' years. '+"{:.1f}".format(100*(timestep+1)/number_of_timesteps)+'% done')
...
np.savetxt('time.out', timearray, delimiter=',') # file that stores time in years at each time step (0D variables are recorded each time step)
np.savetxt('Surf.out', surfcondtimearray, delimiter=',') # rsurf, psurf, Tsurf order
np.savetxt('CMB.out', cmbcondtimearray, delimiter=',') # rhocmb, pcmb, Tcmb order
np.savetxt('Misurf.out', Miatmtimearray, delimiter=',') # Miatm for each time step
np.savetxt('Micore.out', Micoretimearray, delimiter=',') # Micore for each time step

np.savetxt('timeplot.out', timeplotarray, delimiter=',') # file that stores time in years at each time step where plots are produced or 1D variables are recorded
np.savetxt('rk.out', structureplotarray[:,0], delimiter=',') # rows are layer boundaries for a given t (corresponding to timeplot.out)
np.savetxt('pressure.out', structureplotarray[:,1], delimiter=',') # rows are pressure profile for a given t (corresponding to timeplot.out)
np.savetxt('temperature.out', structureplotarray[:,2], delimiter=',') # -||- but temperature profile
np.savetxt('density.out', structureplotarray[:,3], delimiter=',') # -||- but density profile
np.savetxt('Mtot.out', structureplotarray[:,4], delimiter=',') # -||- but total mass in each layer
for i in irange: # save a txt-file for each volatiles' mass distribution in the magma ocean
    np.savetxt('M'+volatiles[i]+'.out', Mvolplotarray[:,i], delimiter=',') # 
...
Mtotfromfile = np.loadtxt('Mtot.out', delimiter=',') # -||- but total mass in each laye
...
# %%%
#-----------------------------Evolution over the loop--------------------------

plt.figure()
for i in irange:
    plt.semilogy(timearray, Micoretimearray[i]/mc0, color=vcolors[i])
...
plt.figure()
for i in irange:
    plt.semilogy(timearray, Miatmtimearray[i], color=vcolors[i])
...
ax2.set_ylabel('Pressure [GPa]', color=colors[0])
ax2.plot(timearray, surfcondtimearray[1,:], color=colors[0])
