...
for timestep in range(1, number_of_timesteps+1):
    ...
    if timestep % gasspacing == 0 or timestep == number_of_timesteps:
        ...
        Miatm, Miatmextra, oxygenmass = gasgasfunc(Miatm, Miatmextra, amuifull, fO2surf, Tsurf) 
        Msil[-1] += oxygenmass # reducing a volatile species lowers the mass. Add the mass to the top layer of the magma ocean. 
        Miatmfull = np.concatenate((Miatm, Miatmextra))
...
if foldersaving:
    variablefoldername = ['Pebble - ', 'Planetesimal - '][planetesimalaccretion]+'SolFac='+str(solubilityfactor)+' - IW'+str(deltaIW)+Ppaperlabel[0]+Ppaperlabel[1]+Ppaperlabel[2]+' - '+str(numEarthoceanscore)+' EO in Core/'
